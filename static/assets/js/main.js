(function ($) {
	...
	function carauselScroll() {
		function initCarouselTicker($ticker, direction) {
			// Avoid double init (plugin wraps <ul> in .carouselTicker__wrap and appends .carouselTicker__clone items)
			if (
				$ticker.find('.carouselTicker__wrap').length ||
				$ticker.find('.carouselTicker__clone').length
			) {
				return;
			}

			// carouselTicker only initializes when content overflows its parent.
			// Ensure overflow by duplicating the list items a few times if needed.
			...
			if ($list.length) {
				...
				if (parentWidth && itemsWidth && itemsWidth <= parentWidth) {
					var copies = 0;
					while (itemsWidth * (copies + 1) <= parentWidth && copies < 3) {
						$items
							.clone()
							.addClass('carouselTicker__prefill')
		...
		$('.carouselTicker-left').each(function () {
			initCarouselTicker($(this), 'prev');
		});
		$('.carouselTicker-right').each(function () {
			initCarouselTicker($(this), 'next');
		});
	}
	...
	function mobileHeaderActive() {
		...
		$offCanvasNavSubMenu
			.parent()
			.prepend('<span class="menu-expand"><i class="arrow-small-down"></i></span>');
