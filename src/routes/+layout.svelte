...
<div class="flex h-full">
    <!-- Left Sidebar -->
    <div
       class="fixed top-0 left-0 h-screen border-2 border-gray-500 rounded-l-md shadowRight z-10 transition-all duration-300 {sidebarExpanded ? 'w-40 sidebar-expanded-background' : 'w-10'}"
       on:mouseenter={() => sidebarExpanded = true}
       on:mouseleave={() => sidebarExpanded = false}
    >
       <Nav {sidebarExpanded}></Nav>
   </div>

    <!-- Main Content -->
    <div class="flex-col flex-1 h-full pl-14 pr-5 md:pl-14 md:pr-14 ">
        <slot />
        <Footer></Footer>
    </div>

    <!-- Right Sidebar with animation -->
<div class="fixed top-0 right-0 h-screen w-10 border-2 border-gray-500 rounded-r-md z-10 hidden md:block">
    ...
    <div class="flex flex-col items-center justify-center h-1/2">
        {#each sections as section (section.id)}
            <a href={`#${section.id}`} class="nav-dot {currentActive === section.id ? 'active' : ''}">&#8226;</a>
        {/each}
    </div>
</div>
