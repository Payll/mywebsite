...
<div class="flex h-full">
    <!-- Left Sidebar -->
    <div class="fixed top-0 left-0 h-screen w-10 border-2 border-gray-500 rounded-l-md shadowRight z-10">
        <Nav></Nav>
    </div>
    ...
    <div class="fixed top-0 right-0 h-screen w-10 border-2 border-gray-500 rounded-r-md z-10">
        ...
        <div class="flex flex-col items-center justify-center h-1/2">
            {#each sections as section (section.id)}
                <a href={`#${section.id}`} class="nav-dot" class:active={currentActive === section.id}>&#8226;</a>
            {/each}
        </div>
    </div>
</div>
