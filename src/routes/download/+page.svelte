        ...
        {#each root as folder}
            <div class="folder">
                <div 
                    ...
                    on:keydown={(event) => event.key === 'Enter' || event.key === ' ' ? toggleFolder(folder.name) : null}
                >
                    <span class="folder-icon">{openFolders[folder.name] ? '📂' : '📁'}</span> <!-- Folder Icon -->
                    {folder.name}
                </div>
    ...
    <div class="w-full md:w-3/4 lg:w-4/5 flex flex-col justify-center items-center bg-gray-200 p-4">
        {#if selectedFile}
            <object data={selectedFile} type="application/pdf" id="pdf-preview" aria-label="PDF Preview">
                <p>Votre navigateur ne supporte pas l'affichage du PDF. Veuillez <a href={selectedFile}>télécharger le fichier</a> pour le visualiser.</p>
            </object>
